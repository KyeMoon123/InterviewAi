...
export const Sidebar = ({setShowModels, showModels}: SidebarProps) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-neutral w-10 lg:w-14 ">
      ...
      <div className={'flex flex-col flex-1 justify-center items-center space-y-12 -mt-30'}>
        <NavLink className={'btn btn-ghost btn-sm'} to={routes.home()} activeClassName={"bg-primary"}>
          <HomeIcon className={'w-5 text-neutral-content'}/>
        </NavLink>
        <NavLink className={'btn btn-ghost btn-sm'} to={routes.chat()} activeClassName={"bg-primary"}>
          <ChatBubbleBottomCenterIcon className={'w-5 text-neutral-content'}/>
        </NavLink>
        <NavLink className={'btn btn-ghost btn-sm'} to={routes.profile()} activeClassName={"bg-primary"}>
            <UserIcon className={'w-5 text-neutral-content'}/>
      ...
      <div className="tooltip tooltip-right" data-tip="Logout">
        <button onClick={() => logOut()}
          ...
          <DoorOpenIcon className={'w-4 text-neutral-content '}/>
        </button>
      </div>
    </div>
  )
